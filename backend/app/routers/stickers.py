...
def get_stickers(db: Session = Depends(get_db), category:Optional[Category] = Query(default = None), 
    ...
    query = (db.query(Sticker,User.gender,User.username,School.id,School.name,School.city,).outerjoin(User, User.id == Sticker.user_id).outerjoin(School, School.id == Sticker.school_id))
    ...
    if date_from:
        ...
        query = query.filter(Sticker.created_at >= start_dt)
    if date_to:
        dto = _parse_date_yyyy_mm_dd(date_to)
        end_dt = datetime(dto.year, dto.month, dto.day, 23, 59, 59)
        query = query.filter(Sticker.created_at <= end_dt)

    rows = query.all()

    # Construir FeatureCollection
    features = []
    for sticker, user_gender, username, school_id, school_name, school_city in rows:    
        ...
        feature["properties"].update({
            ...
            "gender": user_gender,
            "user": {"id": sticker.user_id,"username": username,} if sticker.user_id and username else None,
            "school": {"id": school_id,"name": school_name,"city": school_city,} if school_id else None,
        })
...
def list_comments(sticker_id: int, db: Session = Depends(get_db)):
    ...
    return [
        CommentOut(id=c.id,sticker_id=c.sticker_id,user_id=c.user_id,username=username,content=c.content,created_at=c.created_at,)
...
def add_comment(
    ...
    comment = StickerComment(sticker_id=sticker_id,user_id=user.id,content=content,)

    db.add(comment)
    db.commit()
    db.refresh(comment)

    return CommentOut(id=comment.id,sticker_id=comment.sticker_id,user_id=comment.user_id,username=user.username,content=comment.content,created_at=comment.created_at,)
